import React, { Component } from 'react'
import StickyNav from './StickyNav'
import axios from 'axios'
// import './css/Signup.css'

class Signup extends Component {
  constructor () {
    super()
    this.state = {
      username: '',
      password: ''
    }
  }
  onChange = e => {
    const state = this.state
    state[e.target.name] = e.target.value
    this.setState(state)
  }

  onSubmit = e => {
    e.preventDefault()

    const { username, password, name, ssn, dob } = this.state
    axios
      .post('/api/auth/register', {
        username,
        password,
        ssn,
        name,
        dob: new Date(dob)
      })
      .then(result => {
        this.props.history.push('/login')
      })
  }
  render () {
    const { username, password, ssn, name, dob } = this.state
    return (
      <article class='vh-100 dt w-100 bg-white code'>
        <div class='dtc v-mid tc white ph3 ph4-l'>
          <h1 className='f1 black'>Signup for Spartan Banking!</h1>
          <article class='pa4 black-80 code center'>
            <form
              action='sign-up_submit'
              method='get'
              accept-charset='utf-8'
              onSubmit={this.onSubmit}
            >
              <fieldset id='sign_up' class='ba b--transparent ph0 mh0'>
                <legend class='ph0 mh0 fw6 clip'>Sign Up</legend>
                <div class='mt3'>
                  <label class='db fw4 lh-copy f6' for='email-address'>
                    Email address
                  </label>
                  <input
                    class='form-control pa2 input-reset ba bg-transparent w-100 measure'
                    name='username'
                    id='email-address'
                    type='email'
                    placeholder='Email address'
                    value={username}
                    onChange={this.onChange}
                    required
                  />
                </div>
                <div class='mt3'>
                  <label class='db fw4 lh-copy f6' for='password'>
                    Password
                  </label>
                  <input
                    class='form-control pa2 input-reset ba bg-transparent w-100 measure'
                    type='password'
                    name='password'
                    id='password'
                    placeholder='Password'
                    value={password}
                    onChange={this.onChange}
                    required
                  />
                </div>
                <div class='mt3'>
                  <label class='db fw4 lh-copy f6' for='email-address'>
                    Name
                  </label>
                  <input
                    class='form-control pa2 input-reset ba bg-transparent w-100 measure'
                    name='name'
                    id='name'
                    type='text'
                    placeholder='Name'
                    value={name}
                    onChange={this.onChange}
                    required
                  />
                </div>
                <div class='mt3'>
                  <label class='db fw4 lh-copy f6' for='email-address'>
                    SSN
                  </label>
                  <input
                    class='form-control pa2 input-reset ba bg-transparent w-100 measure'
                    name='ssn'
                    id='ssn'
                    type='text'
                    placeholder='SSN'
                    value={ssn}
                    onChange={this.onChange}
                    required
                  />
                </div>
                <div class='mt3'>
                  <label class='db fw4 lh-copy f6' for='email-address'>
                    Date of Birth
                  </label>
                  <input
                    class='form-control pa2 input-reset ba bg-transparent w-100 measure'
                    name='dob'
                    id='dob'
                    type='text'
                    placeholder='Date of Birth (MM/DD/YYYY)'
                    value={dob}
                    onChange={this.onChange}
                    required
                  />
                </div>
              </fieldset>
              <div class='mt3'>
                <input
                  class='b ph3 pv2 input-reset ba b--black bg-transparent grow pointer f6'
                  type='submit'
                  value='Sign Up'
                />
              </div>
            </form>
          </article>
        </div>
      </article>
    )
  }
}

export default Signup
